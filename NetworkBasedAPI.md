...
If HTTPS can be used for Network based API, browsers can handle a 'localhost' server is trustworthy as well as secure origins on the web. Browsers can also restrict a particular Network based API if critical security vulnerabilities of the secure origin server has not been fixed.
